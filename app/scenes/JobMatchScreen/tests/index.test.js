...
import { MatchJobScreenTest } from '../index';

describe('<MatchJobScreenTest /> Container tests', () => {
  let submitSpy;
  let dispatchMatchRequest;
  let dispatchAcceptDeclineJob;
  ...
  beforeAll(() => {
    submitSpy = jest.fn();
    dispatchMatchRequest = jest.fn();
    dispatchAcceptDeclineJob = jest.fn();
  });

  it('should render and match the snapshot', () => {
    const baseElement = renderProvider(
      <MatchJobScreenTest
        dispatchMatchRequest={dispatchMatchRequest}
        dispatchAcceptDeclineJob={dispatchAcceptDeclineJob}
        loading={false}
        jobs={jobs}
        user={user}
      />
  ...
  it('should fetch the user data on mount', () => {
    ...
    expect(dispatchMatchRequest).toHaveBeenCalled();
  });

  it('should call dispatchAcceptDeclineJob when user presses no thanks button', () => {
    ...
    fireEvent.press(getAllByTestId('no-thanks-button')[0]);
    expect(dispatchAcceptDeclineJob).toHaveBeenCalled();
  });
